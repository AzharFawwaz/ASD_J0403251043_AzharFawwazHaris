...
def tolong(data, first, last):
    if first < last:
        titikBelah = partisi(data, first, last)
        tolong(data, first, titikBelah - 1)
...
def partisi(data, first, last):
    ...
    while not done:
        ...
        while tandakiri <= tandakanan and data[tandakiri] >= nilaiPivot: # "<=" dan ">=" untuk descending
            tandakiri = tandakiri + 1

        while data[tandakanan] <= nilaiPivot and tandakanan >= tandakiri: # "<=" dan ">=" untuk descending
